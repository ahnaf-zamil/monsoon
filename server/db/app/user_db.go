...
type IUserDB interface {
	...
	GetUserByAnyField(ctx context.Context, fields map[db.DBColumn]any) (*api.UserModel, error)
	UpdateUserTableById(ctx context.Context, id int64, table string, values map[db.DBColumn]string) error
...
func (u *UserDB) GetUserByAnyField(ctx context.Context, fields map[db.DBColumn]any) (*api.UserModel, error) {
	...
	cols := []db.DBColumn{tables.ColUserID, tables.ColUserUsername, tables.ColUserDisplayName, tables.ColUserCreatedAt, tables.ColUserEmail, tables.ColUserPassword}
	...
	query := fmt.Sprintf("SELECT %s FROM %s INNER JOIN %s ON %s.%s=%s.%s WHERE ",
		...
		tables.TableUsers,
		tables.TableAuth,
		tables.ColUserID.Table,
		tables.ColUserID.Column,
		tables.ColUserEmail.Table,
		tables.ColUserID.Column)
...
func (u *UserDB) UpdateUserTableById(ctx context.Context, id int64, table string, values map[db.DBColumn]string) error {
	tx, err := u.AppDB.DBPool.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return err
	}
...
func (u *UserDB) GetSessionByAnyField(ctx context.Context, fields map[db.DBColumn]any) (*api.UserSessionModel, error) {
	...
	// Generate the SELECT columns
	cols := []db.DBColumn{tables.ColSessionID, tables.ColSessionUserID, tables.ColSessionRefreshToken, tables.ColSessionCreatedAt}
	selected_columns := db.GenerateDBQueryFields(cols)

	query := fmt.Sprintf("SELECT %s FROM %s WHERE %s",
		selected_columns,
		tables.TableUsersSession,
		or_fields)
...
func (u *UserDB) GetUserAuthByID(ctx context.Context, userID string) (*api.UserAuthModel, error) {
	...
	query := fmt.Sprintf("SELECT %s FROM %s WHERE %s = $1",
		selected_columns,
		tables.TableAuth, tables.ColAuthUserID.Column)
	// The value_arr maintains same sequence of parameters as the columns, which is why we separated the map into two slices
...
func insertUser(tx pgx.Tx, ctx context.Context, id int64, username, display_name string, created_at int64, updated_at int64) error {
	query := fmt.Sprintf("INSERT INTO %s (id, username, display_name, created_at, updated_at) VALUES ($1, $2, $3, $4, $5)", tables.TableUsers)
...
func insertUserAuth(tx pgx.Tx, ctx context.Context, id int64, email string, pw_hash []byte, pwSalt []byte, encSalt []byte, encSeed []byte, nonce []byte) error {
	query := fmt.Sprintf("INSERT INTO %s (id, email, pw_hash, pw_salt, enc_salt, key_seed_cipher, nonce) VALUES ($1, $2, $3, $4, $5, $6, $7)", tables.TableAuth)
...
func insertUserSession(tx pgx.Tx, ctx context.Context, sessionID int64, userID int64, refreshToken string, sessionCreated int64) error {
	query := fmt.Sprintf("INSERT INTO %s (session_id, user_id, refresh_token, created_at) VALUES ($1, $2, $3, $4)", tables.TableUsersSession)
...
func insertUserKey(tx pgx.Tx, ctx context.Context, userID int64, keyEnc []byte, keySig []byte) error {
	query := fmt.Sprintf("INSERT INTO %s (user_id, pub_sig_key, pub_enc_key) VALUES ($1, $2, $3)", tables.TableUsersKey)
